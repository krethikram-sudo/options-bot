...
def slide(bg=PAPER):
    ...
    r = s.shapes.add_shape(1, 0, 0, EMUW, EMUH)  # rectangle
...
def kicker(s, text, color=GRN):
    tf = box(s, 0.9, 0.7, 11.5, 0.5)
    para(tf, text.upper(), 13, color, bold=True, first=True)


def card(s, x, y, w, h, title, body_lines, *, border=LINE, fill=WHITE):
    ...
    tf.margin_top = Inches(0.18); tf.vertical_anchor = MSO_ANCHOR.TOP
    para(tf, title.upper(), 12, MUT, bold=True, first=True, space_after=8)
    for ln, *opt in body_lines:
        para(tf, ln, 13.5, INK, bullet=bool(opt and opt[0] == "b"), space_after=5)
    return sh
...
para(tf, "Maryland is operationalizing its 2025 AI Enablement Strategy.", 32, INK, bold=True, font=SERIF, first=True, space_after=18)
para(tf, "Right after “is it safe?” comes the question every CFO and CIO asks:", 20, MUT, space_after=8)
para(tf, "“What is the AI costing us — per program — and is it on budget?”", 24, GRN, bold=True, space_after=20)
para(tf, "Today that answer lives in invoices and spreadsheets, after the money is spent. Outlay answers it continuously, attributed to the work.", 17, MUT)
...
tf = box(s, 0.9, 1.4, 11.5, 1); para(tf, "Three things, from data you already have.", 30, INK, bold=True, font=SERIF, first=True)
tf = box(s, 0.9, 2.7, 6.6, 4)
for t in [("Attribute — every AI dollar mapped to a team, work type, ticket, and engineer.",),
    ...
    para(tf, t[0], 18, INK, bullet=True, first=(t == ("Attribute — every AI dollar mapped to a team, work type, ticket, and engineer.",)), space_after=12)
card(s, 7.9, 2.7, 4.5, 2.6, "How it connects",
     [("Read-only links to your work tracker (Jira / GitHub / Linear) and your AI provider's usage & billing API.",),
      ("No agents installed. Nothing in the request path.",)])
...
s = slide()
sh = s.shapes.add_shape(1, Inches(0.9), Inches(0.7), Inches(4.6), Inches(0.55))
...
tf = box(s, 0.9, 1.3, 11.5, 1); para(tf, "Straight talk on where we are.", 30, INK, bold=True, font=SERIF, first=True)
card(s, 0.9, 2.5, 5.7, 3.6, "In place today",
     ...
     border=GRN)
card(s, 6.9, 2.5, 5.5, 3.6, "On the roadmap (honest)",
     [("SOC 2 Type II — in progress",), ("Annual pen test — scheduled",),
...
tf = box(s, 0.9, 1.3, 11.5, 1); para(tf, "Two ways this earns its place now.", 30, INK, bold=True, font=SERIF, first=True)
card(s, 0.9, 2.6, 5.7, 2.9, "The AI-governance layer",
     [("As you stand up the 2025 AI strategy, be the cost-attribution + budget-guardrail layer from day one — governance-first, like your program.",)], border=GRN)
card(s, 6.9, 2.6, 5.5, 2.9, "Vendor & enhancement spend",
     [("As the Deloitte / J29 enhancement work adopts AI coding agents, see AI-assisted delivery cost per project — the earned-value view a public agency needs to justify IT spend.",)], border=GRN)
...
# 11 — the ask
s = slide()
sh = s.shapes.add_shape(1, Inches(0.9), Inches(0.7), Inches(2.2), Inches(0.55))
sh.fill.solid(); sh.fill.fore_color.rgb = GRNL; sh.line.fill.background(); sh.shadow.inherit = False
para(sh.text_frame, "THE ASK", 12, GRND, bold=True, first=True)
tf = box(s, 0.9, 1.5, 11.5, 1); para(tf, "A two-week, read-only pilot — zero data-handling risk.", 30, INK, bold=True, font=SERIF, first=True)
tf = box(s, 0.9, 2.8, 11.5, 3)
for t in ["Scoped to your development / vendor AI usage, not any production PHI system.",
    ...
    para(tf, t, 18, INK, bullet=True, first=(t.startswith("Scoped")), space_after=12)
para(tf, "Free during the pilot. We share our SOC 2 timeline and NIST 800-53 control mapping up front.", 15, MUT, space_after=0)
...
# 12 — close
s = slide(GRN); kicker(s, "Why now", color=RGBColor(0xCF,0xEE,0xDE))
tf = box(s, 0.9, 2.3, 11.5, 2.5)
para(tf, "Govern the AI spend before it scales.", 40, WHITE, bold=True, font=SERIF, first=True, space_after=12)
para(tf, "The cheapest time to put AI on a budget is at the start of the curve — which is exactly where Maryland is.", 20, RGBColor(0xDD,0xF0,0xE7))
tf = box(s, 0.9, 6.7, 11.5, 0.5); para(tf, "[presenter] · [email] · outlay-ai.com", 13, RGBColor(0xBF,0xE0,0xD0), first=True)
...
first = True
for q, a in qa:
    para(tf, q, 15, GRN, bold=True, first=first, space_after=2); first = False
    para(tf, a, 13.5, INK, space_after=10)
...
tf = box(s, 0.9, 1.2, 11.8, 1)
para(tf, "Architecture removes the heaviest scope; operational controls are in place; attestations are the roadmap.", 18, INK, bold=True, font=SERIF, first=True)
card(s, 0.9, 2.3, 5.7, 4.2, "Have & can evidence",
     [("Metadata-only / BYOK / read-only — no PHI/PII/FTI",), ("SSO (OIDC) + SCIM",),
      ...
      ("WCAG 2.1 AA + automated a11y & security CI scans",)], border=GRN)
card(s, 6.9, 2.3, 5.5, 4.2, "Roadmap / external / funded",
     [("SOC 2 Type II — in progress (highest leverage)",), ("Annual third-party penetration test",),
      ...
      ("3PAO assessment vs NIST 800-53 Rev 5",)], border=AMBER)
