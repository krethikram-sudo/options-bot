...
cp "$SRC"/PRODUCT_DESIGN.md "$SRC"/ROUTER_TUNING_PLAN.md "$SRC"/SAVINGS_DASHBOARD.md \
   ...
   "$SRC"/PILOT_OUTREACH.md "$SRC"/PILOT_ONEPAGER.md "$SRC"/SPLIT_ARCHITECTURE.md "$SRC"/PLATFORM_RESEARCH.md \
   "$SRC"/LAUNCH_CHECKLIST.md "$SRC"/CONTRACTOR_AGREEMENTS.md "$SRC"/INCIDENT_RESPONSE.md \
   "$SRC"/ICP.md "$SRC"/PROSPECTS.md "$SRC"/PERSONAS_USECASES.md "$SRC"/SELF_OPTIMIZE_EVAL.md "$SRC"/FLEET_LEARNING.md "$SRC"/ENTITY_FORMATION.md "$SRC"/MARKET_STUDY_2026.md "$SRC"/TOKEN_OPTIMIZATION_THESIS.md "$SRC"/LAUNCH_POSTS.md "$SRC"/pilot_tracker.csv "$WORK/internal/"
...
cp "$ROOT"/scripts/publish_modelpilot.sh "$ROOT"/scripts/install_modelpilot_gateway.sh \
   ...
   "$ROOT"/scripts/build_label_worksheet.py \
   "$ROOT"/scripts/md_to_pdf.py "$ROOT"/scripts/make_sales_pdfs.py "$WORK/scripts/"
