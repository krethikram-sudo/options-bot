...
mkdir -p "$DEST/modelpilot/goldenset" "$DEST/tests" "$DEST/extension" \
         "$DEST/scripts" "$DEST/launchd" "$DEST/docs" "$DEST/site" "$DEST/packs"

# --- package code ---
cp "$SRC"/__init__.py "$SRC"/cli.py "$SRC"/gateway.py "$SRC"/enforce.py "$SRC"/router.py \
   "$SRC"/router_classify.py "$SRC"/work_classify.py "$SRC"/client_proxy.py \
   ...
   "$SRC"/logs.py "$SRC"/brain_client.py "$SRC"/sdk.py "$SRC"/retry.py "$SRC"/cache.py "$SRC"/license.py \
...
cp "$ROOT"/packs/*.json "$ROOT"/packs/README.md "$DEST/packs/"
