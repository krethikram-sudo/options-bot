...
screen-by-screen customer flow for **each persona** (Business and Engineering).*
...
> 5. **Business gets a demo-gated sample preview** so a business-led demo isn't a dead end.
>
...
>
> **Review round 4 — business governance depth (this round):**
> 10. **Auto-flagged "Needs your attention" panel** on the business Overview + Summary —
...
>     breaches.
> 12. **Quarterly Summary view** (new business nav item) — an in-app quarter readout:
...
>
> **Review round 5 — business UX consolidation (Phase 1 of 3):**
> 14. **Consolidated business Home** — the near-duplicate Overview + Summary are merged
>     into one opinionated F-pattern Home (attention → KPI scorecard → trust → trend →
>     drill-in cards), so business starts consolidated and drills for detail. Backed by a
>     short design study (`docs/business-ux-proposal.md`) of FinOps/Datadog/Ramp/BI patterns.
...
>     pricing — ranked, one click to the fix. Distinct framing from business's panel.
...
it's built**, **flags runaway tickets**, and lets business **enforce a budget per
...
dollar so the numbers reconcile to the provider invoice and business can trust them.
...
A company's **first** user is either a **Business leader** or an **Engineering
...
| | **Business** (consumes spend after the fact) | **Engineering** (does all the setup) |
|---|---|---|
...
**Key principle (recently enforced):** *Business does no setup.* All connecting,
mapping, and reconciling is engineering's job. Business only consumes dashboards.
...
- Two tiles: **"I'm a business leader setting this up for my business"** /
  **"I'm an engineering leader using this for my business."**
- Business picks the business tile → persona set → advances to F2.

### F2 · Business onboarding (Step 2)

![Business onboarding](images/walkthrough/05_fin_welcome.png)

- H1 **"You're set up as Business."**
...
- **No org upload, no connect form** — business does no setup.
- **"Go to my dashboard →"** (these steps are optional).

### F3 · Business overview — *empty state* (`/app`, before data)

![Business empty state](images/walkthrough/06_fin_overview_empty.png)

This is what business sees until engineering has connected the sources:
...
- **"See it with sample data →"** — a demo-gated preview (founder/demo accounts only)
  so a business prospect can be walked through the real dashboard instead of an empty
  room. Regular business users don't see it (sample data is demo-only by design).
...
### F4 · Business **Home** — the consolidated default (`/app`)

![Business consolidated Home](images/walkthrough/07_fin_overview_data.png)

The single, opinionated business landing (Overview + the old Summary, merged), laid out
...
- **"Needs your attention" panel (act-first, top of page)** — auto-flags what's off
  track so business never has to drill to find it: programs/budgets **already over** or
...
- **Lens bar + saved views** *(Phase 2)* — a control row to **Group by** (team / work
  ...
  type"); pick one as your **default landing**. Per-person, so business and an analyst can
  each have their own.
...
![Business Home — customize mode](images/walkthrough/07d_fin_customize.png)
...
![Business spend page](images/walkthrough/08_fin_spend.png)

- H1 **"Where your AI spend goes."**
- Business-first sections: **Spend by team / cost-center** (the chargeback view) →
...
- The engineering-only "lift your coverage" diagnostic is **hidden** for business.
...
Business does not see Connect or API.)*
...
- **"Share with your business partner"** — invite the business counterpart (no
...
- **"Needs your attention" panel — engineering-focused (act-first).** Auto-flags the
  *operational* things to go fix (not business's budget-governance): **runaway tickets**
...
- **Project burn** — each program/initiative with its **start/end timeline** and a
  **month-by-month** projected-vs-pro-rated-cap burn (breaching months flagged), plus
  the "Define a program" form (start/end dates). Same timeline engine as business's
...
- **Demo mode** — gated to `DEMO_ACCOUNT_EMAILS` (supports `*`, exact emails, and
  whole `@domain`). Lets the founder run a full end-to-end demo (business then
...
signup → **role gate** redirect works · both role tiles present · **business**: welcome
...
welcome (upload direct reports + job title + share-with-business), connect CTA, **has
