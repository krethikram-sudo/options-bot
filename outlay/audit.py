"""Joinability audit — does the Outlay join *convention* hold in the wild?
...
What this proves and doesn't:
  ...
  - It does NOT prove Outlay works end-to-end — that still needs a real
...
Run locally (needs GITHUB_TOKEN):
    ...
    python -m outlay.audit --query "language:python stars:>200 pushed:>2026-05-01" \
...
class GitHubAudit:
    ...
    def _headers(self) -> dict:
        h = {"accept": "application/vnd.github+json", "user-agent": "outlay-audit"}
        if self.token:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Outlay join-convention audit (public repos)")
