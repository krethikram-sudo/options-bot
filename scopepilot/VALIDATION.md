...
ScopePilot's moat is the **branch → ticket join**: resolve the git branch an AI
...
`python -m scopepilot.dogfood --repo krethikram-sudo/options-bot`
...
`python -m scopepilot.audit` measures, across real repos, the fraction of merged
...
4. **The robust foundation is explicit task-tagging** — the launcher/wrapper/CI/
   proxy declares the ticket (`scopepilot/tag.py`), with branch/PR inference and
