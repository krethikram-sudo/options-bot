...
ScopePilot turns AI compute from an unpredictable, runaway line item into
...
planned work costing us, and are we on track?"* ScopePilot is the first platform
...
- **ScopePilot is the platform and primary value prop.** ModelPilot's
  per-request routing is an optimization *service within* it — the actuator
  ScopePilot drives, not a separate product.
