...
Integration (no gateway dependency on ScopePilot):

    export MODELPILOT_REQUEST_OBSERVER=scopepilot.shadow:make_observer
    export SCOPEPILOT_POLICY=policy.json        # from `scopepilot ... --emit-policy`
...
def make_observer() -> Callable[[dict], object]:
    ...
    ledger = ShadowLedger(os.environ.get("SCOPEPILOT_SHADOW_LOG", "scopepilot_shadow.jsonl"))
